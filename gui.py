...
def display(q, el_net, ev, plot_buffer = 3*1e03):
	...
	p_graph = pg.GraphItem(rowspan = 2, col=0)
	...
	p_graph.scene().sigMouseClicked.connect(lambda event: onClick(event, ev, p_graph))
...
def onClick(event, ev, p_network):
	print(event)
	x = p_network.mapSceneToView(event.pos()).x(),
	y = p_network.mapSceneToView(event.pos()).y(),
	ev.clear()
	print (x,y)
